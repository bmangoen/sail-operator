...
func (r *Reconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	// manifestCustomizationHandler handles ManifestCustomizations that target this ZTunnel.
	// The handler triggers reconciliation of the ZTunnel to apply customizations.
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		// +lint-watches:ignore: ManifestCustomization (not found in charts, but must be watched to reconcile ZTunnel when customizations change)
...
// mapManifestCustomizationToReconcileRequests maps ManifestCustomization changes to ZTunnel reconcile requests
func (r *Reconciler) mapManifestCustomizationToReconcileRequests(ctx context.Context, obj client.Object) []reconcile.Request {
	...
	if len(requests) > 0 {
		log.Info("ManifestCustomization change triggers ZTunnel reconciliation",
